...
def get_model():
    ...
    mu = Dense(2, activation='sigmoid')(all_dense_2)
    sigma = Dense(2, activation='softplus')(all_dense_2)
    out_concat = Concatenate()([mu, sigma])
    model = Model(inputs=[face_input, leye_input, reye_input, face_loc_input], outputs=out_concat)
    return model


def get_loss(targets, predictions):
    distributions_x = tfp.distributions.Normal(predictions[:, 0], predictions[:, 2])
    distributions_y = tfp.distributions.Normal(predictions[:, 1], predictions[:, 3])
    log_probs_x = distributions_x.log_prob(targets[:, 0])
    log_probs_y = distributions_y.log_prob(targets[:, 1])
    log_prob = log_probs_x + log_probs_y
